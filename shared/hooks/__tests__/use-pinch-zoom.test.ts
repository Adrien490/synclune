...
describe("usePinchZoom", () => {
	...
	describe("handleKeyDown", () => {
		describe("zoom-in keys (+ / = / *)", () => {
			...
			it("scale is capped at maxScale when zooming in beyond the limit", () => {
				...
				act(() => { result.current.handleKeyDown(makeKeyEvent("+")) }) // 1 → 2
				act(() => { result.current.handleKeyDown(makeKeyEvent("+")) }) // 2 → capped at 2
		...
		describe("zoom-out key (-)", () => {
			it("'-' decreases scale by keyboardZoomStep", () => {
				...
				act(() => { result.current.handleKeyDown(makeKeyEvent("+")) }) // 1 → 1.5
				act(() => { result.current.handleKeyDown(makeKeyEvent("-")) }) // 1.5 → 1
		...
		describe("reset keys (0 / Escape)", () => {
			it("'0' resets scale to minScale", () => {
				...
				act(() => { result.current.handleKeyDown(makeKeyEvent("+")) })
				act(() => { result.current.handleKeyDown(makeKeyEvent("0")) })
	...
	describe("zoomIn", () => {
		...
		it("caps scale at maxScale", () => {
			...
			act(() => { result.current.zoomIn() }) // 1 → 2
			act(() => { result.current.zoomIn() }) // 2 → stays 2
	...
	describe("zoomOut", () => {
		it("decreases scale by keyboardZoomStep", () => {
			...
			act(() => { result.current.zoomIn() }) // 1.5
			act(() => { result.current.zoomOut() }) // back to 1.0
		...
		it("resets position to origin when reaching minScale", () => {
			...
			act(() => { result.current.zoomOut() }) // back to minScale
		...
		it("does not reset position when still above minScale", () => {
			...
			act(() => { result.current.zoomIn() }) // 1.5 → position = {10, 10}
			act(() => { result.current.zoomIn() }) // 2.0 → still above minScale
	...
	describe("reset", () => {
		...
		it("resets to a custom minScale when config.minScale differs from 1", () => {
			const { result } = renderHook(() =>
				usePinchZoom({ containerRef, config: { minScale: 0.5, keyboardZoomStep: 0.5, maxScale: 3 } })
			)

			act(() => { result.current.zoomIn() })    // 0.5 → 1.0
			act(() => { result.current.reset() })
	...
	describe("isActive prop", () => {
		...
		it("does not reset state when isActive is true on initial render", () => {
			const { result } = renderHook(() =>
				usePinchZoom({ containerRef, isActive: true })
			)
		...
		it("resets immediately on the first render when isActive=false", () => {
			const { result } = renderHook(() =>
				usePinchZoom({ containerRef, isActive: false })
			)
	...
	describe("config overrides", () => {
		...
		it("respects custom minScale in reset", () => {
			const { result } = renderHook(() =>
				usePinchZoom({ containerRef, config: { minScale: 0.8, maxScale: 3, keyboardZoomStep: 0.5 } })
			)
