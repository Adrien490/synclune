...
describe("useFabVisibility", () => {
	...
	describe("return value shape", () => {
		it("returns all expected properties", () => {
			const { result } = renderHook(() =>
				useFabVisibility({ key: FAB_KEYS.ADMIN_SPEED_DIAL })
			)
	...
	describe("toggle", () => {
		...
		it("calls onToggle before the server action (immediate invocation)", () => {
			...
			const onToggle = vi.fn(() => { callOrder.push("onToggle") })
	...
	describe("key variants", () => {
		...
		it("works with STOREFRONT key", () => {
			const { result } = renderHook(() =>
				useFabVisibility({ key: FAB_KEYS.STOREFRONT })
			)
		...
		it("works with ADMIN_DASHBOARD key", () => {
			const { result } = renderHook(() =>
				useFabVisibility({ key: FAB_KEYS.ADMIN_DASHBOARD })
			)
