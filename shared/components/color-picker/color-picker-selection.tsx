...
export function ColorPickerSelection({ className, ...props }: ColorPickerSelectionProps) {
	...
	return (
		<div
			...
			aria-describedby="color-picker-selection-desc"
			aria-valuemin={0}
			aria-valuemax={100}
			...
			className={cn(
				"focus-visible:ring-ring relative size-full cursor-crosshair touch-none rounded select-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none",
				className,
			)}
			onPointerDown={(e) => {
				e.preventDefault();
		...
		>
			...
			/>
			<span id="color-picker-selection-desc" className="sr-only">
				Utilisez les flèches pour ajuster. Gauche/Droite: saturation. Haut/Bas: luminosité.
			</span>
		</div>
	);
}
