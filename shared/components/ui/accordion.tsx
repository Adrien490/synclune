...
function AccordionTrigger({ className, children, headingLevel, ...props }: AccordionTriggerProps) {
	...
	const triggerContent = (
		<AccordionPrimitive.Trigger
			...
			className={cn(
				"focus-ring active:bg-muted/50 can-hover:hover:underline flex min-h-11 flex-1 items-center justify-between gap-4 rounded-md py-4 text-left text-sm font-medium disabled:pointer-events-none disabled:opacity-50 [&[data-state=open]>svg]:rotate-180",
				className,
			)}
			{...props}
		>
...
function AccordionContent({
	...
	return (
		<AccordionPrimitive.Content
			...
			forceMount
			className="grid text-sm data-[state=closed]:grid-rows-[0fr] data-[state=open]:grid-rows-[1fr] motion-safe:[transition:grid-template-rows_280ms_var(--ease-premium)]"
			{...props}
		>
