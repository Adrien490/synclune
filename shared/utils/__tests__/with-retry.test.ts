...
describe("withRetry", () => {
	...
	it("should apply exponential backoff between retries", async () => {
		...
		const promise = withRetry(fn, { maxAttempts: 3, baseDelay: 1000, jitter: false }).catch(
			(e) => e,
		);
	...
	it("should throw immediately when isRetryable returns false", async () => {
		...
		await expect(withRetry(fn, { isRetryable, baseDelay: 100 })).rejects.toThrow("non-retryable");
	...
	it("should not delay after the last failed attempt", async () => {
		...
		const promise = withRetry(fn, { maxAttempts: 2, baseDelay: 1000, jitter: false }).catch(
			(e) => e,
		);
