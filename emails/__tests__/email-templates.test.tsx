...
describe("OrderConfirmationEmail", () => {
	...
	it("shows the discount line only when discount is greater than zero", async () => {
		const html = await render(
			<OrderConfirmationEmail {...baseProps} discount={1000} />
		)
...
describe("ShippingConfirmationEmail", () => {
	...
	it("does not show the tracking button when trackingUrl is null", async () => {
		const html = await render(
			<ShippingConfirmationEmail {...baseProps} trackingUrl={null} />
		)
	...
	it("does not show estimated delivery section when not provided", async () => {
		...
		const html = await render(
			<ShippingConfirmationEmail {...propsWithoutDelivery} />
		)
...
describe("PasswordResetEmail", () => {
	...
	it("contains the reset URL in a link", async () => {
		const html = await render(<PasswordResetEmail {...baseProps} />)
		expect(html).toContain(
			"https://synclune.fr/reinitialiser-mot-de-passe?token=abc123"
		)
	})
