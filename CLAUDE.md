...
- **View Transitions : c'est React qui les possède, et le fondu est OPT-IN** — une navigation ne démarre plus la sienne. `withViewTransition(() => router.push(…))` prenait le snapshot d'arrivée AVANT le montage de la page suivante : la transition se jouait entre l'ancienne page et elle-même (34 sites retirés le 2026-08-18, incident `checkout-back-link-viewtransition-bug-2026-05-20`). Les deux layouts posent une frontière `<ViewTransition>` autour de leur `<main>` ; une navigation qui doit fondre le réclame — `router.push(url, PAGE_FADE_NAVIGATION)` ou `transitionTypes` sur son `<Link>` (SSOT `shared/constants/view-transitions.ts`). ⚠️ **Les deux `"none"` de la frontière sont MESURÉS, pas prudentiels** : en `update: "auto"` nu, chaque tronçon streamé par PPR rejoue le fondu (4 transitions enchaînées au premier rendu de `/produits`, en plein chemin du LCP) et l'entrée de la frontière en ajoute une. Le helper impératif ne couvre plus que les mutations d'état **synchrones** (`media-upload-grid`). Verrouillé par `view-transition-navigation.regression.test.ts`, `view-transition-names.regression.test.ts` et `e2e/view-transition-navigation.spec.ts`.
