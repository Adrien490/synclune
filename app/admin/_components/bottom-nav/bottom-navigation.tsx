...
import { getBottomNavPrimaryItems, getAllNavItems } from "../navigation-config";
...
import { Drawer, DrawerContent, DrawerBody, DrawerTitle } from "@/shared/components/ui/drawer";
...
export function BottomNav({ className: _className }: BottomNavigationProps) {
	...
	return (
		<>
					...
					<DrawerBody className="overflow-y-visible pt-2">
						...
						<div className="grid grid-cols-3 gap-2 p-3 pb-2">
							...
							<LogoutAlertDialog>
								<button
									type="button"
									className={cn(panelItemStyles.base, panelItemStyles.destructive, "w-full")}
									aria-label="Deconnexion de votre compte administrateur"
								>
									<LogOut className="size-6 shrink-0" aria-hidden="true" />
									<span className="text-center text-xs leading-tight tracking-tight">
										Deconnexion
									</span>
								</button>
							</LogoutAlertDialog>
			...
			{/* Navigation bar */}
			<nav
				className="bg-background/80 supports-backdrop-filter:bg-background/60 pointer-events-auto fixed right-0 bottom-0 left-0 z-[60] border-t backdrop-blur-lg md:hidden"
				aria-label="Navigation mobile principale"
			>
				<div className="flex h-16 items-center justify-around px-4 pb-[env(safe-area-inset-bottom)]">
					{/* Items principaux */}
					{primaryItems.map((item) => (
						<BottomNavItem key={item.id} item={item} isActive={isRouteActive(pathname, item.url)} />
					))}
					...
					<button
						...
						className={cn(
							"relative flex min-h-12 min-w-16 flex-col items-center justify-center gap-1 rounded-lg px-3 py-2",
					...
					>
						...
						<span className="text-sm leading-none font-medium">Menu</span>
					</button>
				</div>
			</nav>
		</>
