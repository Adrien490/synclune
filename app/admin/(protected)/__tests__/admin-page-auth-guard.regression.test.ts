...
describe("@regression admin-page-self-guard", () => {
	...
	it("every admin page calls assertAdminPage()", () => {
		const offenders = pages.filter((relativePath) => {
			// UNIQUE exemption : la page de connexion, seule page sous /admin hors
			// du groupe (protected) — une garde d'auth sur la porte d'entrée serait
			// un verrou sur la serrure. Elle ne rend aucune donnée admin.
			if (relativePath === "app/admin/connexion/page.tsx") return false;
	...
	it("assertAdminPage delegates to the HMAC-verified session helper", () => {
		// L'invariant ne vaut que si le helper valide réellement le cookie (HMAC +
		// expiry). S'il repassait à une lecture de PRÉSENCE du cookie, les gardes
		// ci-dessus deviendraient décoratives d'un coup — et ce fichier
		// continuerait de passer.
		const source = readFileSync(
			join(REPO_ROOT, "modules/admin-auth/lib/assert-admin-page.ts"),
			"utf-8",
		);

		expect(source).toMatch(
			/import\s+\{[^}]*\bhasValidAdminSession\b[^}]*\}\s+from\s+["']@\/modules\/admin-auth\/lib\/admin-session["']/,
		);
		expect(source).toMatch(/\bhasValidAdminSession\s*\(\s*\)/);
	});

	it("the admin layout keeps its own guard (la page est un filet, pas un remplacement)", () => {
		// Le layout reste la garde du chargement dur : c'est lui qui produit le
		// 401 propre et qui alimente le chrome admin.
		const source = readFileSync(join(REPO_ROOT, "app/admin/(protected)/layout.tsx"), "utf-8");
		expect(source).toMatch(/\bhasValidAdminSession\s*\(\s*\)/);
	});
});
