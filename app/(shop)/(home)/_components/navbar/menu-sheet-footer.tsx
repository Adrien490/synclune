...
/** Cellules « Aide » / « Écrire à Léane » — mêmes surfaces que la bande d'accès rapide. */
...
/**
 * Pied du volet : deux sorties utiles (« Aide » et « Écrire à Léane » —
 * absentes de toute l'interface sous `lg` avant l'étal de poche, P2 de l'audit
 * menu-sheet 2026-08-05) + social + copyright.
 *
 * Les deux liens passent par `useMenuSheetNavigate` (prop contrôlée, jamais
 * `SheetClose` — cf. `menu-sheet-navigate-context`) et portent `replace` :
 ...
 *
 * ⚠️ « Aide » ne pointe plus une page mais l'ANCRE `/#faq` de la landing
 * (absorption de `/aide`, 2026-08-05). Le fragment vise la section FAQ, qui vit
 * dans le shell STATIQUE de l'accueil — hors de toute frontière `Suspense`.
 * C'est ce qui rend le défilement fiable : une cible encore suspendue au moment
 * où le routeur cherche l'ancre laisserait la page en haut, sans erreur.
 */
	...
	return (
		<footer className="relative z-10 shrink-0 px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
			...
			{/* Social + copyright sur UNE rangée : chaque rangée de pied gagnée remonte
			    la bande Collections au-dessus du pli (budget ~630 px de l'artifact). */}
