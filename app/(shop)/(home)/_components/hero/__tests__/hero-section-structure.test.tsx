 ...
 *    un conteneur intermédiaire (ou faire de `HeroGrid` autre chose qu'un
...
vi.mock("../hero-grid", () => ({
	HERO_PRODUCTS_COUNT: 5,
	// Suspend indéfiniment : simule la lecture catalogue la plus lente possible.
	HeroGrid: () => {
		use(neverResolves);
		return null;
	},
	HeroGridSkeleton: () => <div data-testid="hero-grid-skeleton" />,
}));
...
describe("HeroSection — structure", () => {
	it("rend le h1 même quand la grille est encore suspendue", () => {
		render(<HeroSection productsPromise={neverResolves} />);

		const h1 = screen.getByRole("heading", { level: 1 });
		expect(h1).toHaveTextContent("Des bijoux colorés, faits un par un");
		expect(screen.getByTestId("hero-grid-skeleton")).toBeInTheDocument();
	});
	...
	it("place le bloc titre et les cellules dans la même grille", () => {
		...
		const gridCell = screen.getByTestId("hero-grid-skeleton");
	...
	it("relie la section à son titre et n'impose aucune hauteur dérivée de --navbar-height", () => {
		const { container } = render(<HeroSection productsPromise={neverResolves} />);
