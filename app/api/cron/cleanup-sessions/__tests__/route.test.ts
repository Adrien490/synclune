...
vi.mock("@/modules/cron/services/cleanup-sessions.service", () => ({
...
describe("GET /api/cron/cleanup-sessions", () => {
	...
	describe("authorization", () => {
		...
		it("does not call cleanupExpiredSessions when unauthorized", async () => {
		...
		it("proceeds to the service when verifyCronRequest returns null", async () => {
			...
			expect(mockCleanupExpiredSessions).toHaveBeenCalledOnce();
		});
	});
	...
	describe("successful execution", () => {
		...
		it("passes startTime from cronTimer to cronSuccess", async () => {
			mockCronTimer.mockReturnValue(8888);
		...
		it("returns the response from cronSuccess", async () => {
			const successResponse = makeSuccessResponse({ job: "cleanup-sessions" });
	...
	describe("error handling", () => {
		it("calls cronError when cleanupExpiredSessions throws", async () => {
			mockCleanupExpiredSessions.mockRejectedValue(new Error("DB connection lost"));
		...
		it("uses the error message when an Error instance is thrown", async () => {
			mockCleanupExpiredSessions.mockRejectedValue(new Error("Prisma timeout"));
		...
		it("uses fallback message when a non-Error value is thrown", async () => {
			mockCleanupExpiredSessions.mockRejectedValue("unexpected string error");

			await GET();

			expect(mockCronError).toHaveBeenCalledWith("Failed to cleanup sessions");
		});

		it("uses fallback message when null is thrown", async () => {
			mockCleanupExpiredSessions.mockRejectedValue(null);
		...
		it("does not call cronSuccess when the service throws", async () => {
			mockCleanupExpiredSessions.mockRejectedValue(new Error("fail"));
