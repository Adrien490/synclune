...
describe("GET /api/admin/orders/export", () => {
	...
	// ========================================================================
	// Rate limiting (ORD-SEC-001)
	// ========================================================================

	describe("rate limiting", () => {
		it("returns 429 when admin exceeds export rate limit", async () => {
			mockEnforceRateLimit.mockResolvedValue({
				error: {
					status: "error",
					message: "Trop de requêtes. Veuillez réessayer dans 3600 secondes.",
					retryAfter: 3600,
				},
			});
		...
		it("returns the rate-limit message in JSON body", async () => {
			mockEnforceRateLimit.mockResolvedValue({
				error: { status: "error", message: "Trop de requêtes", retryAfter: 60 },
			});
		...
		it("omits Retry-After header when retryAfter is undefined", async () => {
			mockEnforceRateLimit.mockResolvedValue({
