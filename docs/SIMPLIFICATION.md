...
| S2.7 | `VatRegime` (3 valeurs, seul `FRANCHISE_BASE` réel) | ⚠️ **Verdict « garder » PÉRIMÉ par le Lot A** (2026-08-05) : il pesait le besoin futur des 2 autres valeurs, en supposant une colonne pour les porter. Or `Order.vendorVatRegime` est partie avec les 12 colonnes `vendor*` — l'enum n'était plus référencée par aucune colonne ni aucune ligne de TypeScript (le régime est lu depuis l'env en chaîne brute). Droppée au Lot 1 V2 | ~~garder~~ → **retirer** | ✅ 2026-08-05 (audit V2 L1) |
