...
| **Lot 4 — S3.2 ✅ fait (2026-08-03)** | S3.2 rate-limits : ~55 presets admin → `ADMIN_LIMIT` partagé (120/min) + 4 dédiés (export, maintenance, search, invoice-download) ; publics/auth/checkout/webhook conservés granulaires (leçons KI-004) ; presets morts purgés (PDP_WEBHOOK, USER_REFRESH, refunds workflow). **S3.5 dashboard : à suivre** | —         |
