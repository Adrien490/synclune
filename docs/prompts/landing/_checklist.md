...
> Bloc collé à la fin de chaque tour de section. C'est ce qui sépare « l'agent a dessiné » de
...
- [ ] aucun texte ni glyphe posé sur `rose` ou `or` (aplats seuls)
...
- [ ] le haut de la section suivante déborde sous la ligne de flottaison (pas de flèche « scroll »)
      — sans objet si aucune section ne suit (dernière section, frames hors page) : l'écrire
      plutôt que cocher
