/**
 * Script de migration pour générer les placeholders couleur optimisés
 *
 * Ce script :
 ...
 * 4. Génère un SVG gradient ultra-compact (~50 bytes vs ~200-300 bytes base64)
 ...
 *
 * Usage :
 * pnpm exec tsx scripts/generate-color-placeholders.ts
 * pnpm exec tsx scripts/generate-color-placeholders.ts --dry-run       # Pour voir ce qui serait fait
 * pnpm exec tsx scripts/generate-color-placeholders.ts --parallel=10   # Traiter 10 images en parallèle
 * pnpm exec tsx scripts/generate-color-placeholders.ts --json          # Logs JSON pour monitoring
 */
...
const SCRIPT_NAME = "generate-color-placeholders";
...
/** Résultat étendu avec le placeholder couleur */
interface ColorProcessResult extends BaseProcessResult {
	dominantColor?: string;
	blurDataUrlSize?: number;
}
...
/**
 * Traite une image et génère son placeholder couleur
 */
async function processImage(image: MediaItem): Promise<ColorProcessResult> {
	...
	try {
		// Mesure génération placeholder couleur (inclut téléchargement)
		...
		return {
			...
			blurDataUrlSize: placeholder.blurDataUrl.length,
 ...
 */
async function processBatch(images: MediaItem[]): Promise<ColorProcessResult[]> {
...
async function main() {
	logInfo("🎨 Génération des placeholders couleur optimisés\n");
	...
	logInfo(`📊 ${images.length} images trouvées sans placeholder\n`);

	if (images.length === 0) {
		logInfo("✅ Toutes les images ont déjà un placeholder !");
		return;
	}
	...
	for (let i = 0; i < images.length; i += PARALLEL_COUNT) {
		...
		for (const result of results) {
			...
			if (result.success) {
				successCount++;
				if (result.blurDataUrlSize) {
					allSizes.push(result.blurDataUrlSize);
				}
				const timing = result.metrics ? ` (${result.metrics.totalMs}ms)` : "";
				const color = result.dominantColor ? ` ${result.dominantColor}` : "";
				const size = result.blurDataUrlSize ? ` [${result.blurDataUrlSize}B]` : "";
				logInfo(`✅ [${processed}/${images.length}] ${result.id}${color}${size}${timing}`, {
					id: result.id,
					success: true,
					dominantColor: result.dominantColor,
					size: result.blurDataUrlSize,
	...
	const avgSize = allSizes.length > 0 ? Math.round(allSizes.reduce((sum, s) => sum + s, 0) / allSizes.length) : null;
	...
	if (avgSize) {
		logInfo(`\n📦 Taille moyenne des placeholders: ${avgSize} bytes`);
		logInfo(`   (vs ~200-300 bytes pour blur base64 classique)`);
	}

	if (avgMetrics) {
		logInfo(`\n⏱️  Métriques moyennes:`);
		logInfo(`   Génération couleur (download + analyse): ${avgMetrics.blurMs}ms`);
