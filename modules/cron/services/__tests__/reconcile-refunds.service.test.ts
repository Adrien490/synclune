...
describe("reconcileRefunds", () => {
	...
	// ========================================================================
	// Le bloc « overbilling auto-resolution » (8 tests) a été retiré avec la passe
	// `reconcileOverbilledOrders` (audit du module orders, 2026-08-05) : elle ne
	// servait qu'à éteindre un compteur de dashboard, via une heuristique de
	// rattachement refund↔overbilling qui n'existe pas en base.
	//
	// ⚠️ Ce qui reste testé ailleurs, et qui compte : la DÉTECTION du trop-perçu
	// (`payment-handlers`), la persistance d'`overbilledAmountCents` et la garde
	// anti-sur-remboursement qui le lit (`refund-handlers`, `order.total + overbilled`).
});
