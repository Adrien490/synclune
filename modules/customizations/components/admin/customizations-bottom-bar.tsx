...
import { SORT_LABELS } from "../../constants/sort.constants";
...
/**
 * Barre d'actions fixe en bas pour mobile (tri, recherche, filtre statut).
 *
 * 3 boutons:
 * - Trier → ouvre SortDrawer
 * - Rechercher → ouvre un drawer avec input
 * - Filtrer → ouvre CustomizationsStatusDrawer
 *
 * Visible uniquement mobile (md:hidden).
 * Se cache quand un sheet/dialog est ouvert (meme pattern que ProductSortBar).
 */
export function CustomizationsBottomBar() {
	...
	// Hide when any sheet/dialog is open (same pattern as ProductSortBar)
	...
	// (SidebarInset has `relative` which can trap `fixed` when combined with transforms)
	...
	return createPortal(
		<>
			...
			>
				...
				>
					...
					<button
						...
						aria-label={
							hasActiveFilter ? "Filtre statut actif. Modifier le filtre" : "Ouvrir les filtres"
						}
						aria-haspopup="dialog"
					>
			...
			<CustomizationsStatusDrawer open={filterOpen} onOpenChange={setFilterOpen} />
		</>,
		document.body,
	);
}
