 ...
 */
export function ReviewSummaryCompact({ stats, className }: ReviewSummaryCompactProps) {
	...
	return (
		...
		>
			<RatingStars rating={stats.averageRating} maxRating={5} size="sm" />
			<span className="text-sm font-medium">{formatRating(stats.averageRating)}</span>
			<span className="text-muted-foreground text-sm">({stats.totalCount} avis)</span>
		</div>
	);
}
