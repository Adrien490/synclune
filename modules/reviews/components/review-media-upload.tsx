...
export function ReviewMediaUpload({
	...
	return (
		<div className={cn("space-y-3", className)} aria-busy={isUploading} aria-live="polite">
			{/* Aperçu des photos */}
			{media.length > 0 && (
				<div className="flex flex-wrap gap-2" role="list" aria-label="Photos ajoutées">
					{media.map((m, index) => (
						<div
							key={m.url}
							role="listitem"
								...
								<div className="flex flex-col items-center gap-2 py-2">
									...
									<div className="text-center">
										...
										<p className="text-muted-foreground text-xs">
											{REVIEW_CONFIG.MAX_MEDIA_COUNT - media.length} restante
											{REVIEW_CONFIG.MAX_MEDIA_COUNT - media.length > 1 ? "s" : ""} (max 4 Mo)
										</p>
									</div>
								</div>
							),
