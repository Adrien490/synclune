...
vi.mock("../../constants/review.constants", () => ({
	REVIEW_ERROR_MESSAGES: {
		...
		RESPONSE_DELETE_FAILED: "Erreur suppression réponse",
	},
}));
vi.mock("../../schemas/review.schemas", () => ({
	deleteReviewResponseSchema: { safeParse: mockSafeParse },
}));

import { deleteReviewResponse } from "../delete-review-response";
...
describe("deleteReviewResponse", () => {
	beforeEach(() => {
		...
		mockSafeParse.mockReturnValue({ success: true, data: { id: RESPONSE_ID } });
	...
	it("returns auth error when user is not admin", async () => {
		...
		const result = await deleteReviewResponse(undefined, createMockFormData({ id: RESPONSE_ID }));
	...
	it("returns validation error when schema fails", async () => {
		...
		const result = await deleteReviewResponse(undefined, createMockFormData({ id: "" }));
	...
	it("returns notFound when response does not exist", async () => {
		...
		expect(mockNotFound).toHaveBeenCalledWith("Réponse");
	});

	it("soft-deletes the response", async () => {
		await deleteReviewResponse(undefined, createMockFormData({ id: RESPONSE_ID }));
