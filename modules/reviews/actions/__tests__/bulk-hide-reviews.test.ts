...
vi.mock("../../schemas/review.schemas", () => ({
	bulkHideReviewsSchema: { safeParse: mockSafeParse },
}));
vi.mock("../../services/review-stats.service", () => ({
	updateProductReviewStats: mockUpdateProductReviewStats,
}));

import { bulkHideReviews } from "../bulk-hide-reviews";
...
describe("bulkHideReviews", () => {
	...
	it("returns auth error when user is not admin", async () => {
		...
		const result = await bulkHideReviews(undefined, createMockFormData({ ids: "[]" }));
	...
	it("returns error when no reviews found", async () => {
		...
		const _result = await bulkHideReviews(
			undefined,
	...
	it("sets status to HIDDEN for all reviews in transaction", async () => {
		await bulkHideReviews(undefined, createMockFormData({ ids: JSON.stringify(REVIEW_IDS) }));
	...
	it("recalculates stats for each affected product", async () => {
		...
		expect(mockUpdateProductReviewStats).toHaveBeenCalledWith(mockPrisma, "prod-2");
	});

	it("deduplicates product IDs before recalculating stats", async () => {
		mockPrisma.productReview.findMany.mockResolvedValue([
			{ id: "rev-1", productId: "prod-1" },
			{ id: "rev-2", productId: "prod-1" },
		]);
		await bulkHideReviews(undefined, createMockFormData({ ids: JSON.stringify(REVIEW_IDS) }));
	...
	it("returns success with count", async () => {
		const result = await bulkHideReviews(
			undefined,
			createMockFormData({ ids: JSON.stringify(REVIEW_IDS) }),
		);
