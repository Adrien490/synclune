...
describe("moderateReview", () => {
	...
	it("should toggle PUBLISHED to HIDDEN", async () => {
		...
		expect(mockSuccess).toHaveBeenCalledWith("Avis masqué avec succès", { status: "HIDDEN" });
	});
	...
	it("should toggle HIDDEN to PUBLISHED", async () => {
		...
		expect(result.status).toBe(ActionStatus.SUCCESS);
		expect(mockSuccess).toHaveBeenCalledWith("Avis republié avec succès", { status: "PUBLISHED" });
	});
	...
	it("should call handleActionError on unexpected exception", async () => {
		...
		const result = await moderateReview(undefined, validFormData);
		expect(mockHandleActionError).toHaveBeenCalledWith(expect.any(Error), "Erreur modération");
		expect(result.status).toBe(ActionStatus.ERROR);
	});
});
