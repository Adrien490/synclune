...
import {
	quickSearchReviewsAdmin,
	type AdminQuickSearchReviewItem,
} from "../data/quick-search-reviews-admin";

import type { AdminQuickSearchResult } from "@/shared/components/sticky-action-bar";

const querySchema = z.string().trim().min(1).max(100);

export async function quickSearchReviewsAdminAction(
	query: string,
): Promise<AdminQuickSearchResult<AdminQuickSearchReviewItem>> {
	...
	const result = await quickSearchReviewsAdmin(parsed.data);
