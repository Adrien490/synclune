...
import { bulkDeleteReviewsSchema } from "../schemas/review.schemas"
...
/**
 * Supprime plusieurs avis en masse (soft delete)
...
export async function bulkDeleteReviews(
	...
	try {
		...
		const validation = bulkDeleteReviewsSchema.safeParse(rawData)
		...
		// 3. Récupérer les avis pour connaître leurs productIds et médias
		...
		// Invalider aussi le cache user pour chaque utilisateur affecté
		const userIds = [...new Set(reviews.map((r) => r.userId).filter((id): id is string => id !== null))]
		userIds.forEach((userId) => {
			tagsToInvalidate.add(REVIEWS_CACHE_TAGS.USER(userId))
			tagsToInvalidate.add(REVIEWS_CACHE_TAGS.REVIEWABLE(userId))
		})
		...
		// 6. Supprimer les fichiers UploadThing (fire-and-forget)
		const allMediaUrls = reviews.flatMap((r) => r.medias.map((m) => m.url))
		if (allMediaUrls.length > 0) {
			deleteUploadThingFilesFromUrls(allMediaUrls).catch((err) => {
				console.error("[bulkDeleteReviews] Erreur suppression fichiers UploadThing:", err)
			})
		}

		return success(`${reviews.length} avis supprimé(s) avec succès`, { count: reviews.length })
	} catch (e) {
