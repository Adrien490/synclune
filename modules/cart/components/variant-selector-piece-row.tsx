...
export interface VariantSelectorPieceRowProps {
	variant: ActiveVariant;
	/** Prix effectif (override variante ?? prix produit) — calculé par le parent. */
	priceCents: number;
...
/**
 * Une ligne = une pièce réelle (un VARIANT), pas un attribut.
 *
 ...
 */
export function VariantSelectorPieceRow({
	variant,
	priceCents,
	label,
	image,
	...
	return (
		<button
			...
			aria-label={`${fullLabel}, ${formatEuro(priceCents)}, ${stock.label}`}
			onClick={() => {
				if (isSoldOut) return;
				onSelect();
			}}
