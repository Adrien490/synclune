...
vi.mock("next/dynamic", () => ({
	default: (_fn: () => Promise<{ CartSheet?: unknown; VariantSelectorDialog?: unknown }>) => {
...
import { CartAndVariantWrapper } from "../cart-and-variant-wrapper";
...
describe("CartAndVariantWrapper", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	});
	...
	it("renders the cart sheet skeleton as Suspense fallback during SSR", () => {
		...
		expect(() => render(<CartAndVariantWrapper />)).not.toThrow();
	});

	it("renders without crashing", async () => {
		const { container } = render(<CartAndVariantWrapper />);
