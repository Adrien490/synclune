...
import type { VariantSelectorDialogData } from "../types/dialog-data.types";
import { VARIANT_SELECTOR_DIALOG_ID } from "./variant-selector-utils";
import { VariantSelectorPieces } from "./variant-selector-pieces";

export type { VariantSelectorDialogData };
...
/**
 * Dialog de choix de variante, ouvert depuis une carte produit à plusieurs VARIANT.
 *
 * Direction « Une pièce à la fois » (audit design 2026-08-04) : une ligne par VARIANT
...
export function VariantSelectorDialog({
	...
	const { isOpen, data, close } = useDialog<VariantSelectorDialogData>(VARIANT_SELECTOR_DIALOG_ID);
	...
	// Le store n'est jamais ouvert sans produit : `openVariantSelector({ product })` les
	...
	const description =
		activeVariants.length === 0
			? VARIANT_SELECTOR_TEXTS.NO_ACTIVE_VARIANTS
			: allSoldOut
				? VARIANT_SELECTOR_TEXTS.ALL_SOLD_OUT
				: VARIANT_SELECTOR_TEXTS.pieceCount(activeVariants.length);
	...
	return (
		<ResponsiveDialog open={isOpen} onOpenChange={(open) => !open && close()}>
			{/* ⚠️ `md:flex md:flex-col md:overflow-y-hidden` : le DialogContent desktop est
			    ...
			    flex-col. */}
			<ResponsiveDialogContent className="group/variant-selector md:flex md:max-h-[85vh] md:max-w-130 md:flex-col md:overflow-y-hidden">
