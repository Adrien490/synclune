...
describe("removeUnavailableItems", () => {
	...
	it("should invalidate product FOMO cache tags for each removed item's product", async () => {
		const unavailableItem1 = createMockCartItem({ id: "item-1", sku: { id: "sku-1", inventory: 0, isActive: true, deletedAt: null, product: { id: "prod-1", title: "P1", status: "PUBLIC", deletedAt: null } } })
		const unavailableItem2 = createMockCartItem({ id: "item-2", sku: { id: "sku-2", inventory: 0, isActive: true, deletedAt: null, product: { id: "prod-2", title: "P2", status: "PUBLIC", deletedAt: null } } })
		mockPrisma.cart.findFirst.mockResolvedValue(createMockCart([unavailableItem1, unavailableItem2]))
	...
	it("should deduplicate product cache tags when multiple items share the same product", async () => {
		...
		const item1 = createMockCartItem({ id: "item-1", sku: { id: "sku-1", inventory: 0, isActive: true, deletedAt: null, product } })
		const item2 = createMockCartItem({ id: "item-2", sku: { id: "sku-2", inventory: 0, isActive: true, deletedAt: null, product } })
		...
		const productCacheCalls = vi.mocked(mockUpdateTag).mock.calls.filter(
			([tag]) => tag === "product-carts-prod-shared"
		)
