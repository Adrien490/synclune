...
import { createMockFormData, createMockOrder, VALID_CUID, VALID_USER_ID, VALID_ORDER_ID } from "@/test/factories"
...
describe("updateOrderShippingAddress", () => {
	beforeEach(() => {
		...
		mockGetOrderMetadataInvalidationTags.mockReturnValue(["orders-list", `orders-user-${VALID_USER_ID}`])
	...
	it("should invalidate order metadata cache tags", async () => {
		...
		expect(mockGetOrderMetadataInvalidationTags).toHaveBeenCalledWith(VALID_USER_ID, VALID_ORDER_ID)
