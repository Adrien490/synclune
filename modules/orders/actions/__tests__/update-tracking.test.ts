...
vi.mock("@/shared/lib/actions", () => ({
	validateInput: (schema: { safeParse: (data: unknown) => unknown }, data: unknown) => {
		const result = schema.safeParse(data) as { success: boolean; data?: unknown; error?: { issues: { message: string }[] } }
		if (!result.success) {
			return {
				error: {
...
describe("updateTracking", () => {
	...
	it("should return error when order is not SHIPPED or DELIVERED", async () => {
		mockPrisma.order.findUnique.mockResolvedValue(
			createShippedOrder({ status: "PROCESSING" })
		)
	...
	it("should succeed when order is DELIVERED", async () => {
		mockPrisma.order.findUnique.mockResolvedValue(
			createShippedOrder({ status: "DELIVERED" })
		)
