...
vi.mock("@/modules/emails/services/order-emails", () => ({ sendDeliveryConfirmationEmail: mockSendDeliveryEmail }))
vi.mock("@/modules/webhooks/services/review-request.service", () => ({ scheduleReviewRequestEmail: mockScheduleReviewEmail }))
...
describe("markAsDelivered", () => {
	beforeEach(() => {
		...
		vi.mocked(markAsDeliveredSchema.safeParse).mockReturnValue({
			success: true, data: { id: VALID_CUID, sendEmail: true },
		} as never)
	...
	it("should return auth error when not admin", async () => {
		mockRequireAdminWithUser.mockResolvedValue({ error: { status: ActionStatus.UNAUTHORIZED, message: "No" } })
