...
vi.mock("@/modules/auth/lib/require-auth", () => ({ requireAdminWithUser: mockRequireAdminWithUser }))
...
const validFormDataWithReason = createMockFormData({ id: VALID_CUID, reason: "Produit defectueux" })
...
describe("markAsReturned", () => {
	beforeEach(() => {
		...
		vi.mocked(markAsReturnedSchema.safeParse).mockReturnValue({
			success: true, data: { id: VALID_CUID, reason: undefined },
		} as never)
	...
	it("should return auth error when not admin", async () => {
		mockRequireAdminWithUser.mockResolvedValue({ error: { status: ActionStatus.UNAUTHORIZED, message: "Non autorise" } })
		const result = await markAsReturned(undefined, validFormData)
		expect(result.status).toBe(ActionStatus.UNAUTHORIZED)
	})

	it("should return rate limit error when rate limited", async () => {
		mockEnforceRateLimit.mockResolvedValue({ error: { status: ActionStatus.ERROR, message: "Trop de requetes" } })
		const result = await markAsReturned(undefined, validFormData)
	...
	it("should return validation error for invalid ID", async () => {
		vi.mocked(markAsReturnedSchema.safeParse).mockReturnValue({
			success: false, error: { issues: [{ message: "ID invalide" }] },
		} as never)
		const result = await markAsReturned(undefined, validFormData)
	...
	it("should return error already_returned when fulfillmentStatus is RETURNED", async () => {
		...
		mockPrisma.order.findUnique.mockResolvedValue(createDeliveredOrder({ fulfillmentStatus: "RETURNED" }))
		const result = await markAsReturned(undefined, validFormData)
		expect(result.status).toBe(ActionStatus.ERROR)
	...
	it("should create audit trail with reason when provided", async () => {
		vi.mocked(markAsReturnedSchema.safeParse).mockReturnValue({
			success: true, data: { id: VALID_CUID, reason: "Produit defectueux" },
		} as never)
