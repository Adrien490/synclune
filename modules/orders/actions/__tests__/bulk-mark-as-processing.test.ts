...
vi.mock("../../schemas/order.schemas", () => ({
	bulkMarkAsProcessingSchema: {},
}));

import { bulkMarkAsProcessing } from "../bulk-mark-as-processing";
...
describe("bulkMarkAsProcessing", () => {
	...
	it("should return auth error when not admin", async () => {
		...
		const result = await bulkMarkAsProcessing(undefined, makeFormData());
	...
	it("should return error for malformed JSON ids", async () => {
		...
		const result = await bulkMarkAsProcessing(undefined, formData);
	...
	// No eligible orders
	it("should return error when no eligible PENDING + PAID orders found", async () => {
	...
	// Status update
	it("should update eligible PENDING orders to PROCESSING status", async () => {
		const order = createPendingPaidOrder();
		mockPrisma.order.findMany.mockResolvedValue([order]);
	...
	it("should create an audit trail entry for each order", async () => {
		const order1 = createPendingPaidOrder({ id: "order-1" });
		const order2 = createPendingPaidOrder({ id: "order-2", orderNumber: "SYN-2026-0002" });
		mockPrisma.order.findMany.mockResolvedValue([order1, order2]);

		await bulkMarkAsProcessing(undefined, makeFormData());
	...
	it("should invalidate cache tags for each unique userId", async () => {
		const order1 = createPendingPaidOrder({ id: "order-1", userId: "user-1" });
		const order2 = createPendingPaidOrder({
			id: "order-2",
			userId: "user-2",
