...
import { createMockFormData, createMockOrder, VALID_ORDER_ID, VALID_USER_ID } from "@/test/factories"
...
vi.mock("../../constants/order.constants", () => ({
	ORDER_ERROR_MESSAGES: {
		BULK_DELETE_NONE_ELIGIBLE: "Aucune commande ne peut etre supprimee (toutes ont des factures ou sont payees).",
...
describe("bulkDeleteOrders", () => {
	...
	it("should treat missing ids field as empty array and pass to validateInput", async () => {
		const validationError = { status: ActionStatus.VALIDATION_ERROR, message: "Au moins une commande" }
	...
	it("should return error when no deletable orders found", async () => {
		...
		mockPrisma.order.findMany.mockResolvedValue([
			...
			createDeletableOrder({ id: "order-2", invoiceNumber: "INV-002", orderNumber: "SYN-2026-0002" }),
		])
	...
	it("should skip orders with an invoiceNumber", async () => {
		...
		const skipped = createDeletableOrder({ id: "order-skipped", invoiceNumber: "INV-001", orderNumber: "SYN-2026-0002" })
	...
	it("should skip orders with PAID payment status", async () => {
		...
		const skipped = createDeletableOrder({ id: "order-paid", paymentStatus: "PAID", orderNumber: "SYN-2026-0002" })
		mockPrisma.order.findMany.mockResolvedValue([deletable, skipped])
	...
	it("should skip orders with REFUNDED payment status", async () => {
		const deletable = createDeletableOrder({ id: "order-deletable", paymentStatus: "PENDING" })
		const skipped = createDeletableOrder({ id: "order-refunded", paymentStatus: "REFUNDED", orderNumber: "SYN-2026-0002" })
		mockPrisma.order.findMany.mockResolvedValue([deletable, skipped])
	...
	it("should include skipped count in success message when some orders were skipped", async () => {
		...
		const skipped = createDeletableOrder({ id: "order-2", invoiceNumber: "INV-001", orderNumber: "SYN-2026-0002" })
		mockPrisma.order.findMany.mockResolvedValue([deletable, skipped])
	...
	it("should invalidate cache tags for each unique userId", async () => {
		...
		const order2 = createDeletableOrder({ id: "order-2", userId: "user-2", orderNumber: "SYN-2026-0002" })
