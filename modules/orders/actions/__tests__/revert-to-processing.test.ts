...
vi.mock("../../schemas/order.schemas", () => ({
	revertToProcessingSchema: {
		safeParse: vi.fn().mockReturnValue({ success: true, data: { id: VALID_CUID, reason: "Erreur transporteur" } }),
	},
}))
...
describe("revertToProcessing", () => {
	beforeEach(() => {
		...
		vi.mocked(revertToProcessingSchema.safeParse).mockReturnValue({
			success: true, data: { id: VALID_CUID, reason: "Erreur transporteur" },
		} as never)
	...
	it("should return validation error when reason is missing", async () => {
		vi.mocked(revertToProcessingSchema.safeParse).mockReturnValue({
			success: false, error: { issues: [{ message: "La raison est obligatoire" }] },
		} as never)
