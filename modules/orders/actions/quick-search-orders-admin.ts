...
import {
	quickSearchOrdersAdmin,
	type AdminQuickSearchOrderItem,
} from "../data/quick-search-orders-admin";

import type { AdminQuickSearchResult } from "@/shared/components/sticky-action-bar";

const querySchema = z.string().trim().min(1).max(100);

export async function quickSearchOrdersAdminAction(
	query: string,
): Promise<AdminQuickSearchResult<AdminQuickSearchOrderItem>> {
	...
	const result = await quickSearchOrdersAdmin(parsed.data);
