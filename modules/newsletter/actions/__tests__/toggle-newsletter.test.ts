...
describe("toggleNewsletter", () => {
	...
	it("should return unauthorized when user is not logged in", async () => {
		const unauthorizedError = { status: ActionStatus.UNAUTHORIZED, message: "Vous devez être connecté." }
	...
	it("should return error when rate limit is exceeded", async () => {
		const rateLimitError = { status: ActionStatus.ERROR, message: "Trop de requêtes. Réessayez plus tard.", data: { retryAfter: 3600 } }
	...
	it("should return validation error when action value is invalid", async () => {
		...
		const result = await toggleNewsletter(undefined, createMockFormData({ action: "invalid_action" }))
