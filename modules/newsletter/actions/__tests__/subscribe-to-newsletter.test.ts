...
describe("subscribeToNewsletter", () => {
	...
	it("should return error when Arcjet rate limit is denied", async () => {
		...
		const result = await subscribeToNewsletter(undefined, createMockFormData({ email: "user@example.com", consent: "true" }))
	...
	it("should return validation error when input is invalid", async () => {
		...
		const result = await subscribeToNewsletter(undefined, createMockFormData({ email: "not-an-email", consent: "false" }))
	...
	it("should return CONFLICT status when subscriber is already confirmed", async () => {
		...
		expect(result.message).toBe("Si cette adresse n'est pas encore inscrite, un email de confirmation vous a été envoyé.")
	})
	...
	it("should pass email, ip and userAgent to the internal function", async () => {
		...
		await subscribeToNewsletter(undefined, createMockFormData({ email: "confirmed@example.com", consent: "true" }))
