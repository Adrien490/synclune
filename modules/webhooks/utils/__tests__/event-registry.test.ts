...
// ============================================================================
// dispatchEvent - payment_intent handlers (flow Elements, seul flow réel)
// ============================================================================
...
describe("dispatchEvent - payment_intent handlers", () => {
	...
	it("should route payment_intent.succeeded to handlePaymentSuccess with payment intent object", async () => {
		const paymentIntent = { id: "pi_test_1", metadata: { orderId: "order-1" } };
	...
	it("should route payment_intent.payment_failed to handlePaymentFailure with payment intent object", async () => {
		const paymentIntent = { id: "pi_test_failed", metadata: { orderId: "order-2" } };
	...
	it("should route payment_intent.canceled to handlePaymentCanceled with payment intent object", async () => {
		const paymentIntent = { id: "pi_test_canceled", metadata: { orderId: "order-3" } };
	...
	it("should route payment_intent.processing to handlePaymentProcessing with payment intent object", async () => {
		const paymentIntent = { id: "pi_test_processing", metadata: { orderId: "order-4" } };
		const event = makeEvent("payment_intent.processing", paymentIntent);
		mockHandlePaymentProcessing.mockResolvedValue(mockResult);

		const result = await dispatchEvent(event);

		expect(mockHandlePaymentProcessing).toHaveBeenCalledOnce();
		expect(mockHandlePaymentProcessing).toHaveBeenCalledWith(paymentIntent);
		expect(result).toEqual(mockResult);
	});
});
...
describe("dispatchEvent - unsupported events", () => {
	...
	it("should not call any handler for unsupported events", async () => {
		...
		expect(mockHandlePaymentProcessing).not.toHaveBeenCalled();
...
describe("dispatchEvent - error propagation", () => {
	...
	it("should propagate errors thrown by the payment success handler", async () => {
		const handlerError = new Error("PI processing failed");
		const event = makeEvent("payment_intent.succeeded", { id: "pi_test_err" });
		mockHandlePaymentSuccess.mockRejectedValue(handlerError);

		await expect(dispatchEvent(event)).rejects.toThrow("PI processing failed");
	});

	it("should propagate errors from payment failure handler", async () => {
		const handlerError = new Error("No orderId in payment intent metadata");
		...
		await expect(dispatchEvent(event)).rejects.toThrow("No orderId in payment intent metadata");
	});
...
describe("isEventSupported", () => {
	it("should return true for all supported event types (flow Elements + refund/dispute/invoice)", () => {
	...
	it("should return false for retired Checkout Session events (flow hosted retiré)", () => {
		expect(isEventSupported("checkout.session.completed")).toBe(false);
		expect(isEventSupported("checkout.session.expired")).toBe(false);
		expect(isEventSupported("checkout.session.async_payment_succeeded")).toBe(false);
		expect(isEventSupported("checkout.session.async_payment_failed")).toBe(false);
	});
	...
	it("should act as a type guard (TypeScript narrowing)", () => {
		const eventType: string = "payment_intent.succeeded";
