...
const { mockAdjustVariantStock } = vi.hoisted(() => ({
	mockAdjustVariantStock: vi.fn(),
}));

vi.mock("@/modules/variants/actions/adjust-variant-stock", () => ({
	adjustVariantStock: mockAdjustVariantStock,
}));
...
describe("useAdjustStockForm", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		mockAdjustVariantStock.mockResolvedValue({
	...
	it("returns form, state, action, isPending, adjustment, newStock, and isValid", () => {
		const { result } = renderHook(() =>
			useAdjustStockForm({ variantId: "variant-1", currentStock: 10 }),
		);
