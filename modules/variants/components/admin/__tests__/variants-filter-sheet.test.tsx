...
describe("VariantsFilterSheet", () => {
	...
	it("renders color section when colorOptions are provided", () => {
		render(<VariantsFilterSheet colorOptions={colorOptions} materialOptions={[]} />);
	...
	it("renders material section when materialOptions are provided", () => {
		render(<VariantsFilterSheet colorOptions={[]} materialOptions={materialOptions} />);
	...
	it("renders separators between filter groups", () => {
		render(<VariantsFilterSheet colorOptions={colorOptions} materialOptions={materialOptions} />);
		const separators = screen.getAllByTestId("separator");
		expect(separators.length).toBeGreaterThan(0);
	});

	it("forwards className to FilterSheetWrapper as triggerClassName", () => {
		render(<VariantsFilterSheet className="custom-class" colorOptions={[]} materialOptions={[]} />);
	...
	it("calls handleChange with 'active' when the active radio option is selected", () => {
		...
		expect(mockFieldHandleChange).toHaveBeenCalledWith("active", "active");
	});
	...
	it("calls handleChange with 'inactive' when the inactive radio option is selected", () => {
		...
		expect(mockFieldHandleChange).toHaveBeenCalledWith("active", "inactive");
	});
	...
	it("calls handleChange with 'all' when the all radio option is selected", () => {
		...
		expect(mockFieldHandleChange).toHaveBeenCalledWith("active", "all");
	});
	...
	it("initializes active='active' when filter_isActive=true", () => {
	...
	it("initializes active='inactive' when filter_isActive=false", () => {
	...
	it("does not bubble the form submit event", () => {
		...
		render(
			<div onSubmit={parentSubmit}>
				<VariantsFilterSheet colorOptions={[]} materialOptions={[]} />
			</div>,
		);
	...
	it("encodes active=active as filter_isActive=true", async () => {
		render(<VariantsFilterSheet colorOptions={[]} materialOptions={[]} />);
		await capturedOnSubmit?.({
			value: { stockStatuses: [], colorIds: [], materialIds: [], active: "active" },
		});
		const pushedUrl = mockRouterPush.mock.calls[0]?.[0] as string;
		expect(pushedUrl).toContain("filter_isActive=true");
	});

	it("encodes active=inactive as filter_isActive=false", async () => {
		render(<VariantsFilterSheet colorOptions={[]} materialOptions={[]} />);
		await capturedOnSubmit?.({
			value: { stockStatuses: [], colorIds: [], materialIds: [], active: "inactive" },
		});
		const pushedUrl = mockRouterPush.mock.calls[0]?.[0] as string;
		expect(pushedUrl).toContain("filter_isActive=false");
	});

	it("does not add filter_isActive to URL when active=all", async () => {
		render(<VariantsFilterSheet colorOptions={[]} materialOptions={[]} />);
		await capturedOnSubmit?.({
			value: { stockStatuses: [], colorIds: [], materialIds: [], active: "all" },
		});
	...
	it("replaces existing filter params with new values when applyFilters is called", async () => {
		...
		render(<VariantsFilterSheet colorOptions={[]} materialOptions={[]} />);
		await capturedOnSubmit?.({
			value: { stockStatuses: ["out_of_stock"], colorIds: [], materialIds: [], active: "all" },
		});
		const pushedUrl = mockRouterPush.mock.calls[0]?.[0] as string;
