...
import { ColorsBottomBar } from "../colors-bottom-bar";
...
describe("ColorsBottomBar", () => {
	...
	describe("active states", () => {
		...
		describe("search active state", () => {
			it("shows active dot when search param is set", () => {
				mockSearchParams.set("search", "rouge");
				renderDefault();
				expect(screen.queryAllByTestId("active-dot").length).toBeGreaterThanOrEqual(1);
			});
			...
			it("uses active aria-label on search button when search is active", () => {
				...
				renderDefault();
				expect(
					screen.getByRole("button", { name: /Recherche: "rouge"\. Modifier la recherche/ }),
				).toBeInTheDocument();
			});
			...
			it("shows clear button in search drawer when search is active", () => {
				mockSearchParams.set("search", "bleu");
				renderDefault();
				fireEvent.click(screen.getByRole("button", { name: /Recherche: "bleu"/ }));
		...
		it("shows multiple active dots when both sort and search are active simultaneously", () => {
			mockSearchParams.set("sortBy", "name-ascending");
			mockSearchParams.set("search", "rouge");
			renderDefault();
	...
	describe("button clicks", () => {
		...
		it("clicking menu button toggles nav menu open", () => {
			renderDefault();
			const menuButton = screen.getByRole("button", { name: /Ouvrir le menu de navigation/ });
			expect(menuButton).toHaveAttribute("aria-expanded", "false");
			fireEvent.click(menuButton);
			// The mock open fn is called — aria-expanded stays false as mockIsMenuOpen is hoisted constant
	...
	describe("search form", () => {
		it("submitting search form calls router.push with search param", () => {
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir la recherche/ }));
			const input = screen.getByRole("searchbox", { name: /Rechercher une couleur/ });
			fireEvent.change(input, { target: { value: "rouge" } });
			const form = input.closest("form")!;
			fireEvent.submit(form);
			expect(mockPush).toHaveBeenCalledOnce();
			const url = mockPush.mock.calls[0]![0] as string;
			expect(url).toContain("search=rouge");
		});
		...
		it("submitting search form removes cursor and direction params", () => {
			...
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir la recherche/ }));
			const input = screen.getByRole("searchbox", { name: /Rechercher une couleur/ });
			fireEvent.change(input, { target: { value: "bleu" } });
			const form = input.closest("form")!;
			fireEvent.submit(form);
			const url = mockPush.mock.calls[0]![0] as string;
		...
		it("submitting empty search removes search param", () => {
			mockSearchParams.set("search", "rouge");
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Recherche: "rouge"/ }));
			const input = screen.getByRole("searchbox", { name: /Rechercher une couleur/ });
		...
		it("search input is pre-filled with current search param value", () => {
			mockSearchParams.set("search", "vert");
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Recherche: "vert"/ }));
			const input = screen.getByRole("searchbox", {
				name: /Rechercher une couleur/,
			}) as HTMLInputElement;
			expect(input.defaultValue).toBe("vert");
		});

		it("clicking clear button removes search param and closes drawer", () => {
			mockSearchParams.set("search", "jaune");
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Recherche: "jaune"/ }));
		...
		it("search input has correct placeholder", () => {
			...
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir la recherche/ }));
			const input = screen.getByRole("searchbox", { name: /Rechercher une couleur/ });
			expect(input).toHaveAttribute("placeholder", "Nom, slug, hex...");
		});
	});
	...
	describe("sort drawer options", () => {
		it("renders all sort options from COLORS_SORT_LABELS inside sort drawer", () => {
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir les options de tri/ }));
			expect(screen.getByText("Nom (A-Z)")).toBeInTheDocument();
			expect(screen.getByText("Nom (Z-A)")).toBeInTheDocument();
			expect(screen.getByText("Moins de SKU")).toBeInTheDocument();
			expect(screen.getByText("Plus de SKU")).toBeInTheDocument();
		});
