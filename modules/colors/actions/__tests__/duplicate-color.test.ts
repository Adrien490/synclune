...
vi.mock("@/shared/lib/rate-limit-config", () => ({ ADMIN_COLOR_LIMITS: { DUPLICATE: "color-duplicate" } }))
...
describe("duplicateColor", () => {
	beforeEach(() => {
		...
		mockPrisma.color.create.mockResolvedValue({ id: "color-2", name: "Or Rose (copie)", slug: "or-rose-copie" })
	...
	it("should return error when unique name generation fails", async () => {
		mockGenerateUniqueReadableName.mockResolvedValue({ success: false, error: "Trop de copies existantes" })
		const result = await duplicateColor(undefined, validFormData)
		expect(result.status).toBe(ActionStatus.ERROR)
