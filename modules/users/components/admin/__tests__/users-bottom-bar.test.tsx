...
vi.mock("../users-filter-sheet", () => ({
	UsersFilterSheet: ({
		...
		<div data-testid="filter-sheet" data-open={open} data-hide-trigger={hideTrigger}>
			{open && <div data-testid="filter-sheet-content">Filtrer les clients</div>}
...
vi.mock("../../constants/user.constants", () => ({
	USERS_SORT_LABELS: {
		"createdAt-descending": "Plus récents",
		"createdAt-ascending": "Plus anciens",
		"updatedAt-descending": "Mis à jour récemment",
		"updatedAt-ascending": "Mis à jour anciennement",
		"name-ascending": "Nom (A-Z)",
		"name-descending": "Nom (Z-A)",
		"email-ascending": "Email (A-Z)",
		"email-descending": "Email (Z-A)",
	},
}));
...
describe("UsersBottomBar", () => {
	describe("rendering", () => {
		...
		it("renders the four action buttons: Filtrer, Trier, Rechercher, Menu", () => {
			renderDefault();
	...
	describe("toolbar a11y", () => {
		...
		it("all four buttons have aria-haspopup='dialog'", () => {
			renderDefault();
	...
	describe("roving tabindex", () => {
		...
		it("updates focused index back to filter button on focus", () => {
			renderDefault();
			const searchButton = screen.getByRole("button", { name: /Ouvrir la recherche/ });
			fireEvent.focus(searchButton);
			const filterButton = screen.getByRole("button", { name: /Ouvrir les filtres/ });
	...
	describe("keyboard navigation", () => {
		it("ArrowRight from filter button (0) moves focus to sort button (1)", () => {
			renderDefault();
			const filterButton = screen.getByRole("button", { name: /Ouvrir les filtres/ });
		...
		it("ArrowRight from sort button (1) moves focus to search button (2)", () => {
			renderDefault();
			const sortButton = screen.getByRole("button", { name: /Ouvrir les options de tri/ });
			fireEvent.focus(sortButton);
		...
		it("ArrowRight from search button (2) moves focus to menu button (3)", () => {
			renderDefault();
			const searchButton = screen.getByRole("button", { name: /Ouvrir la recherche/ });
			fireEvent.focus(searchButton);
		...
		it("ArrowRight from menu button (3) wraps to filter button (0)", () => {
			renderDefault();
			const menuButton = screen.getByRole("button", { name: /Ouvrir le menu de navigation/ });
			fireEvent.focus(menuButton);
		...
		it("ArrowLeft from filter button (0) wraps to menu button (3)", () => {
			renderDefault();
			const filterButton = screen.getByRole("button", { name: /Ouvrir les filtres/ });
		...
		it("ArrowLeft from sort button (1) moves focus to filter button (0)", () => {
			renderDefault();
			const sortButton = screen.getByRole("button", { name: /Ouvrir les options de tri/ });
			fireEvent.focus(sortButton);
		...
		it("ArrowLeft from search button (2) moves focus to sort button (1)", () => {
			renderDefault();
			const searchButton = screen.getByRole("button", { name: /Ouvrir la recherche/ });
			fireEvent.focus(searchButton);
		...
		it("ArrowLeft from menu button (3) moves focus to search button (2)", () => {
			renderDefault();
			const menuButton = screen.getByRole("button", { name: /Ouvrir le menu de navigation/ });
			fireEvent.focus(menuButton);
		...
		it("ArrowUp behaves like ArrowLeft", () => {
			...
			const sortButton = screen.getByRole("button", { name: /Ouvrir les options de tri/ });
			fireEvent.focus(sortButton);
			fireEvent.keyDown(sortButton, { key: "ArrowUp" });
			const filterButton = screen.getByRole("button", { name: /Ouvrir les filtres/ });
			expect(filterButton).toHaveAttribute("tabindex", "0");
		});

		it("Home key moves focus to first button (filter)", () => {
			renderDefault();
			const menuButton = screen.getByRole("button", { name: /Ouvrir le menu de navigation/ });
			fireEvent.focus(menuButton);
	...
	describe("active states", () => {
		describe("sort active state", () => {
			it("shows active dot when sortBy param is set", () => {
				mockSearchParams.set("sortBy", "createdAt-descending");
				renderDefault();
		...
		describe("search active state", () => {
			...
			it("uses active aria-label on search button when search is active", () => {
				...
				renderDefault();
				expect(
					screen.getByRole("button", { name: /Recherche: "alice"\. Modifier la recherche/ }),
				).toBeInTheDocument();
			});
			...
			it("shows clear button in search drawer when search is active", () => {
				mockSearchParams.set("search", "alice");
				renderDefault();
				fireEvent.click(screen.getByRole("button", { name: /Recherche: "alice"/ }));
		...
		describe("filter active state", () => {
			it("shows active dot when a filter_ param is set", () => {
				mockSearchParams.set("filter_role", "ADMIN");
				renderDefault();
				expect(screen.queryAllByTestId("active-dot").length).toBeGreaterThanOrEqual(1);
			});

			it("shows active dot when filter_status is set", () => {
				mockSearchParams.set("filter_status", "ACTIVE");
				renderDefault();
				expect(screen.queryAllByTestId("active-dot").length).toBeGreaterThanOrEqual(1);
			});

			it("shows active dot when filter_hasOrders is set", () => {
				mockSearchParams.set("filter_hasOrders", "true");
				renderDefault();
				expect(screen.queryAllByTestId("active-dot").length).toBeGreaterThanOrEqual(1);
			});

			it("uses 'Filtres actifs' aria-label on filter button when filter is active", () => {
				mockSearchParams.set("filter_role", "ADMIN");
				renderDefault();
				expect(
					screen.getByRole("button", { name: /Filtres actifs\. Modifier les filtres/ }),
				).toBeInTheDocument();
			});
			...
			it("does not treat non-filter_ params as active filters", () => {
				mockSearchParams.set("sortBy", "name-ascending");
				mockSearchParams.set("search", "alice");
				renderDefault();
				expect(screen.getByRole("button", { name: /Ouvrir les filtres/ })).toBeInTheDocument();
			});
	...
	describe("button clicks", () => {
		it("clicking filter button opens filter sheet", () => {
			renderDefault();
			const filterButton = screen.getByRole("button", { name: /Ouvrir les filtres/ });
			fireEvent.click(filterButton);
			expect(screen.getByTestId("filter-sheet")).toHaveAttribute("data-open", "true");
			expect(screen.getByTestId("filter-sheet-content")).toBeInTheDocument();
		});
		...
		it("opening filter sheet closes sort drawer", () => {
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir les options de tri/ }));
			expect(screen.getByTestId("sort-drawer")).toHaveAttribute("data-open", "true");
			const filterButton = screen.getByRole("button", { name: /Ouvrir les filtres/ });
		...
		it("opening filter sheet closes search drawer", () => {
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir la recherche/ }));
			expect(screen.getByTestId("drawer-title")).toBeInTheDocument();
			const filterButton = screen.getByRole("button", { name: /Ouvrir les filtres/ });
	...
	describe("search form", () => {
		it("search input has aria-label 'Rechercher un client'", () => {
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir la recherche/ }));
		...
		it("search input has placeholder 'Nom, email...'", () => {
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir la recherche/ }));
			const input = screen.getByRole("searchbox", { name: /Rechercher un client/ });
			expect(input).toHaveAttribute("placeholder", "Nom, email...");
		});
	...
	describe("hide logic", () => {
		...
		it("passes isHidden=true when filter sheet is open", () => {
			renderDefault();
	...
	describe("sort drawer options", () => {
		it("renders all sort options from USERS_SORT_LABELS inside sort drawer", () => {
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir les options de tri/ }));
			expect(screen.getByText("Plus récents")).toBeInTheDocument();
			expect(screen.getByText("Plus anciens")).toBeInTheDocument();
			expect(screen.getByText("Mis à jour récemment")).toBeInTheDocument();
			expect(screen.getByText("Mis à jour anciennement")).toBeInTheDocument();
			expect(screen.getByText("Nom (A-Z)")).toBeInTheDocument();
			expect(screen.getByText("Nom (Z-A)")).toBeInTheDocument();
			expect(screen.getByText("Email (A-Z)")).toBeInTheDocument();
			expect(screen.getByText("Email (Z-A)")).toBeInTheDocument();
		});
