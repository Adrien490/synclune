...
import { MaterialsBottomBar } from "../materials-bottom-bar";
...
function renderDefault() {
	return render(<MaterialsBottomBar />);
}
...
describe("MaterialsBottomBar", () => {
	...
	describe("toolbar a11y", () => {
		...
		it("menu button has aria-expanded attribute", () => {
			renderDefault();
			const menuButton = screen.getByRole("button", { name: /menu de navigation/ });
	...
	describe("active states", () => {
		...
		describe("search active state", () => {
			it("shows active dot when search param is set", () => {
				mockSearchParams.set("search", "argent");
				renderDefault();
				expect(screen.queryAllByTestId("active-dot").length).toBeGreaterThanOrEqual(1);
			});
			...
			it("uses active aria-label on search button when search is active", () => {
				...
				renderDefault();
				expect(
					screen.getByRole("button", { name: /Recherche: "argent"\. Modifier la recherche/ }),
				).toBeInTheDocument();
			});
			...
			it("shows clear button in search drawer when search is active", () => {
				mockSearchParams.set("search", "or");
				renderDefault();
				fireEvent.click(screen.getByRole("button", { name: /Recherche: "or"/ }));
	...
	describe("search form", () => {
		it("submitting search form calls router.push with search param", () => {
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir la recherche/ }));
			const input = screen.getByRole("searchbox", { name: /Rechercher un materiau/ });
			fireEvent.change(input, { target: { value: "argent" } });
			const form = input.closest("form")!;
			fireEvent.submit(form);
			expect(mockPush).toHaveBeenCalledOnce();
			const url = mockPush.mock.calls[0]![0] as string;
			expect(url).toContain("search=argent");
		});
		...
		it("submitting empty search removes search param", () => {
			mockSearchParams.set("search", "argent");
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Recherche: "argent"/ }));
			const input = screen.getByRole("searchbox", { name: /Rechercher un materiau/ });
		...
		it("search input is pre-filled with current search param value", () => {
			mockSearchParams.set("search", "cuivre");
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Recherche: "cuivre"/ }));
			const input = screen.getByRole("searchbox", {
				name: /Rechercher un materiau/,
			}) as HTMLInputElement;
			expect(input.defaultValue).toBe("cuivre");
		});

		it("clicking clear button removes search param and closes drawer", () => {
			mockSearchParams.set("search", "titane");
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Recherche: "titane"/ }));
		...
		it("search input has correct placeholder", () => {
			...
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir la recherche/ }));
			const input = screen.getByRole("searchbox", { name: /Rechercher un materiau/ });
			expect(input).toHaveAttribute("placeholder", "Nom, slug, description...");
		});
	});
	...
	describe("sort drawer options", () => {
		it("renders all sort options from MATERIALS_SORT_LABELS inside sort drawer", () => {
			renderDefault();
			fireEvent.click(screen.getByRole("button", { name: /Ouvrir les options de tri/ }));
