...
import {
	quickSearchMaterialsAdmin,
	type AdminQuickSearchMaterialItem,
} from "../data/quick-search-materials-admin";

import type { AdminQuickSearchResult } from "@/shared/components/sticky-action-bar";

const querySchema = z.string().trim().min(1).max(100);

export async function quickSearchMaterialsAdminAction(
	query: string,
): Promise<AdminQuickSearchResult<AdminQuickSearchMaterialItem>> {
	...
	const result = await quickSearchMaterialsAdmin(parsed.data);
