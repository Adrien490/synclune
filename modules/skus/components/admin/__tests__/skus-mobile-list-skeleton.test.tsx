...
import { SkusMobileListSkeleton } from "../skus-mobile-list-skeleton";

afterEach(cleanup);

describe("SkusMobileListSkeleton", () => {
	it("renders SkeletonGroup with the loading label", () => {
		const { getByTestId } = render(<SkusMobileListSkeleton />);
		expect(getByTestId("skeleton-group")).toHaveAttribute("aria-label", "Chargement des variantes");
	});

	it("renders 5 item placeholders", () => {
		const { getAllByTestId } = render(<SkusMobileListSkeleton />);
		expect(getAllByTestId("item")).toHaveLength(5);
	});

	it("wrapper is hidden on md", () => {
		const { container } = render(<SkusMobileListSkeleton />);
