...
const MAX_PRIMARY_IMAGE_SIZE = 16 * 1024 * 1024;
...
export function SkuPrimaryImageField({
	...
	const validateAndUpload = async (files: File[]) => {
		...
		if (file.size > MAX_PRIMARY_IMAGE_SIZE) {
			triggerHaptic("error");
			toast.error("L'image dépasse la limite de 16 Mo");
			return;
		}
			...
			<PrimaryImageUpload
				...
				renderUploadZone={() => (
					<UploadActionSheet
						...
						onFilesSelected={validateAndUpload}
						triggerLabel="Définir l'image principale"
						triggerDescription="Image • Max 16 Mo"
						sheetTitle="Image principale"
						sheetDescription="Choisissez la photo qui s'affichera en premier sur la fiche produit"
						showCamera
						desktopFallback={
										...
										label: ({ isDragActive, isUploading: uploading }) => {
											...
											return (
												<div className="space-y-2 text-center">
													...
													<p className="text-muted-foreground mt-2 text-xs">Image • Max 16 Mo</p>
												</div>
											);
										},
										allowedContent: () => null,
