"use client";

import {
	ArrowRightIcon,
	ListDashesIcon,
	PlusIcon,
	StarIcon,
	WarningIcon,
} from "@phosphor-icons/react/ssr";
import Link from "next/link";

import { STOCK_THRESHOLDS } from "@/shared/constants/cache-tags";
import { Badge } from "@/shared/components/ui/badge";
import { Button } from "@/shared/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/shared/components/ui/card";
import { useHaptic } from "@/shared/hooks/use-haptic";
import { formatEuro } from "@/shared/utils/format-euro";
import { buildVariantLabel } from "@/modules/variants/utils/variant-variant-label";
import type { GetProductReturn } from "@/modules/products/types/product.types";

const PREVIEW_LIMIT = 3;

interface ProductDetailVariantsSummaryCardProps {
	product: GetProductReturn;
}

export function ProductDetailVariantsSummaryCard({
	product,
}: ProductDetailVariantsSummaryCardProps) {
	const haptic = useHaptic();
	const variants = product.variants;
	const variantsCount = variants.length;
	const totalStock = variants.reduce((sum, variant) => sum + variant.stock, 0);
	// Prix effectif : override variante ?? prix produit (schéma lean)
	const prices = variants.map((variant) => variant.priceCents ?? product.priceCents);
	const minPrice = prices.length > 0 ? Math.min(...prices) : product.priceCents;
	const maxPrice = prices.length > 0 ? Math.max(...prices) : product.priceCents;
	const outOfStockCount = variants.filter((variant) => variant.stock === 0).length;

	// Les VARIANTs arrivent triés par (position, id) : le rang 0 est la variante
	// principale, l'aperçu prend donc les premiers tels quels.
	const previewVariants = variants.slice(0, PREVIEW_LIMIT);
	const remainingCount = Math.max(0, variants.length - PREVIEW_LIMIT);

	const priceLabel =
		minPrice === maxPrice
			? formatEuro(minPrice)
			: `${formatEuro(minPrice)} – ${formatEuro(maxPrice)}`;

	const stockVariant: "destructive" | "warning" | "success" =
		totalStock === 0 || totalStock <= STOCK_THRESHOLDS.CRITICAL
			? "destructive"
			: totalStock <= STOCK_THRESHOLDS.LOW
				? "warning"
				: "success";

	return (
		<Card style={{ viewTransitionName: "product-detail-variants" }}>
			<CardHeader>
				<CardTitle className="flex items-center gap-2">
					<ListDashesIcon className="size-5" aria-hidden="true" />
					Variantes
				</CardTitle>
			</CardHeader>
			<CardContent className="space-y-4">
				<dl className="grid gap-3 text-sm">
					<div className="flex items-center justify-between gap-3">
						<dt className="text-muted-foreground">Variantes actives</dt>
						<dd className="font-medium">{variantsCount}</dd>
					</div>
					<div className="flex items-center justify-between gap-3">
						<dt className="text-muted-foreground">Stock total</dt>
						<dd>
							<Badge variant={stockVariant} aria-label={`${totalStock} en stock`}>
								{totalStock}
							</Badge>
						</dd>
					</div>
					<div className="flex items-center justify-between gap-3">
						<dt className="text-muted-foreground">Prix</dt>
						<dd className="flex items-center gap-2">
							<span className="font-medium">{priceLabel}</span>
						</dd>
					</div>
				</dl>

				{outOfStockCount > 0 ? (
					<p className="text-destructive flex items-center gap-1.5 text-xs" role="status">
						<WarningIcon className="size-3.5 shrink-0" aria-hidden="true" />
						{outOfStockCount} variante{outOfStockCount > 1 ? "s" : ""} en rupture
					</p>
				) : null}

				{previewVariants.length > 0 ? (
					<ul className="-mx-2 space-y-0.5 border-t pt-3" aria-label="Aperçu des variantes">
						{previewVariants.map((variant, index) => {
							const label = buildVariantLabel(variant) || `Variante ${index + 1}`;
							const isOutOfStock = variant.stock === 0;
							return (
								<li
									key={variant.id}
									className="flex items-center justify-between gap-3 rounded-md px-2 py-1.5 text-sm"
								>
									<div className="flex min-w-0 items-center gap-1.5">
										{/* Rang 0 = variante principale (plus de flag isDefault) */}
										{index === 0 ? (
											<StarIcon
												className="text-muted-foreground size-3 shrink-0"
												aria-label="Variante principale"
											/>
										) : null}
										<span className="truncate">{label}</span>
									</div>
									<div className="flex shrink-0 items-center gap-1.5">
										<Badge
											variant={isOutOfStock ? "destructive" : "secondary"}
											aria-label={`${variant.stock} en stock`}
										>
											{isOutOfStock ? "Rupture" : `${variant.stock}`}
										</Badge>
									</div>
								</li>
							);
						})}
					</ul>
				) : null}

				{remainingCount > 0 ? (
					<p className="text-muted-foreground text-xs">
						+ {remainingCount} autre{remainingCount > 1 ? "s" : ""} variante
						{remainingCount > 1 ? "s" : ""}
					</p>
				) : null}

				<div className="flex flex-col gap-2 sm:flex-row">
					<Button
						render={
							<Link
								href={`/admin/catalogue/produits/${product.slug}/variantes/nouveau`}
								onClick={() => haptic("light")}
							/>
						}
						className="flex-1 touch-manipulation transition-transform duration-150 active:scale-[0.98]"
					>
						<PlusIcon className="size-4" aria-hidden="true" />
						Nouvelle variante
					</Button>
					<Button
						render={
							<Link
								href={`/admin/catalogue/produits/${product.slug}/variantes`}
								onClick={() => haptic("light")}
							/>
						}
						variant="outline"
						className="flex-1 touch-manipulation transition-transform duration-150 active:scale-[0.98]"
					>
						Gérer les variantes
						<ArrowRightIcon className="size-4" aria-hidden="true" />
					</Button>
				</div>
			</CardContent>
		</Card>
	);
}
