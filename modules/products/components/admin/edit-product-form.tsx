...
export function EditProductForm({
	...
	// Track original image URLs for deletion tracking
	...
	return (
		<>
			...
			>
			{/* Champs cachés pour l'ID produit et SKU */}
			...
			{/* Champ caché pour les URLs d'images à supprimer */}
			<form.Subscribe
				selector={(state) => [
			...
			>
				{([primaryImage, galleryMedia]) => {
					...
					// Find deleted URLs (original URLs not in current URLs)
			...
			<FormLayout cols={2}>
				...
				<FormSection
					...
					description="L'image principale qui représentera votre produit dans les listings et résultats de recherche"
				>
					<form.Field
						name="defaultSku.primaryImage"
						validators={{
					...
					<form.AppField name="status">
						{(field) => (
							<div className="space-y-2">
								...
								/>
								<p className="text-xs text-muted-foreground">
									⚠️ Archiver un produit désactive automatiquement tous ses SKUs
								</p>
							</div>
						)}
					</form.AppField>
			...
			<FormLayout cols={2}>
				...
				<FormSection
					title="Prix et disponibilité (SKU par défaut)"
					description="Configuration du prix et du stock du SKU par défaut"
				>
					{/* Prix TTC */}
					<form.AppField
					...
					<form.AppField name="defaultSku.compareAtPriceEuros">
						{(field) => (
							<div className="space-y-2">
								...
								<p className="text-xs text-muted-foreground">
									Optionnel. Affichera le prix barré si supérieur au prix de
									vente
								</p>
							</div>
						)}
					...
					<form.AppField name="defaultSku.inventory">
						{(field) => (
							<div className="space-y-2">
								...
								<p className="text-xs text-muted-foreground">
									Laissez vide ou 0 si le produit est en rupture
								</p>
							</div>
						)}
					</form.AppField>
				...
				<FormSection
					...
					description="Images et vidéos supplémentaires du produit (jusqu'à 10 médias)"
				>
										...
										<div className="flex items-center gap-3 py-4 px-4 text-left bg-muted/20 rounded-lg border border-dashed border-border">
											...
											<div>
												...
												<p className="text-xs text-muted-foreground mt-0.5">
													...
													montrer votre produit sous différents angles
												</p>
											</div>
										</div>
									)}
