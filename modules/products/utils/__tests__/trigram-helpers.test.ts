...
describe("setTrigramThreshold", () => {
	...
	it("clamps threshold above 1 to 1", async () => {
		...
		expect(tx.$executeRawUnsafe).toHaveBeenCalledWith("SET LOCAL pg_trgm.similarity_threshold = 1");
		expect(tx.$executeRawUnsafe).toHaveBeenCalledWith(
	...
	it("clamps negative threshold to 0", async () => {
		...
		expect(tx.$executeRawUnsafe).toHaveBeenCalledWith("SET LOCAL pg_trgm.similarity_threshold = 0");
	});
...
describe("setStatementTimeout", () => {
	it("sets statement_timeout with valid milliseconds", async () => {
		...
		expect(tx.$executeRawUnsafe).toHaveBeenCalledWith("SET LOCAL statement_timeout = '2000ms'");
	});

	it("rounds fractional milliseconds", async () => {
		const tx = createMockTx();
		await setStatementTimeout(tx as any, 1500.7);
		expect(tx.$executeRawUnsafe).toHaveBeenCalledWith("SET LOCAL statement_timeout = '1501ms'");
	});

	it("clamps negative values to 0", async () => {
		const tx = createMockTx();
		await setStatementTimeout(tx as any, -100);
		expect(tx.$executeRawUnsafe).toHaveBeenCalledWith("SET LOCAL statement_timeout = '0ms'");
	});
