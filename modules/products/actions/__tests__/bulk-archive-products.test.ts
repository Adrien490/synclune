...
vi.mock("@/shared/lib/rate-limit-config", () => ({ ADMIN_PRODUCT_BULK_ARCHIVE_LIMIT: "admin-product-bulk-archive" }))
...
describe("bulkArchiveProducts", () => {
	...
	it("should not deactivate SKUs when unarchiving (DRAFT target)", async () => {
		...
		const unarchiveFormData = createMockFormData({ productIds: JSON.stringify(productIds), targetStatus: "DRAFT" })
