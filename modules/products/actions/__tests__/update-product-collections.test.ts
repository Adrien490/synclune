...
vi.mock("@/shared/lib/rate-limit-config", () => ({ ADMIN_PRODUCT_UPDATE_COLLECTIONS_LIMIT: "admin-product-update-collections" }))
...
describe("updateProductCollections", () => {
	...
	it("should return not found when some collections do not exist", async () => {
		...
		mockNotFound.mockReturnValue({ status: ActionStatus.NOT_FOUND, message: "Collections introuvables" })
		const result = await updateProductCollections(undefined, validFormData)
		expect(result.status).toBe(ActionStatus.NOT_FOUND)
	})
