...
import { createProductSkuSchema } from "../schemas/product-sku.schemas";
...
export async function createProductSku(
	...
	try {
		...
		const productSku = await prisma.$transaction(async (tx) => {
			...
			// CONTRAINTE METIER : Verifier l'unicite de la combinaison (productId, colorId, size, material)
			...
			if (existingCombination) {
				const variantDetails = [
					...
					normalizedMaterial ? `materiau "${normalizedMaterial}"` : null,
				]
					.filter(Boolean)
					.join(", ");
