...
export function generateHighlights(product: GetProductReturn): ProductHighlight[] {
	...
	const primaryMaterial = product.variants[0]?.material;
	...
	const hasAdjustableSize = product.variants.some((variant) =>
		variant.size?.toLowerCase().includes("ajustable"),
	);
	...
	const activeCollection = product.collections.find((collection) => collection.active);
	if (activeCollection) {
