...
function params(overrides: Partial<GetDiscountsParams> = {}): GetDiscountsParams {
	return { perPage: 20, sortBy: "created-descending", ...overrides } as GetDiscountsParams;
}
...
describe("buildDiscountWhereClause", () => {
	...
	describe("search", () => {
		it("should add a case-insensitive code search condition when search is provided", () => {
			const result = buildDiscountWhereClause(params({ search: "SUMMER" }));
		...
		it("should not add a search condition when search is an empty string", () => {
			const result = buildDiscountWhereClause(params({ search: "" }));
	...
	describe("filter by isActive", () => {
		...
		it("should not add an isActive condition when filters.isActive is undefined", () => {
			const result = buildDiscountWhereClause(params({ filters: {} }));
