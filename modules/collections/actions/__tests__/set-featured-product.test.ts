...
vi.mock("@/shared/lib/rate-limit-config", () => ({ ADMIN_COLLECTION_LIMITS: { DELETE: "col-delete", UPDATE: "col-update", BULK_DELETE: "col-bulk-delete" } }))
...
describe("setFeaturedProduct", () => {
	beforeEach(() => {
		...
		mockValidateInput.mockReturnValue({ data: { collectionId: COLLECTION_ID, productId: PRODUCT_ID } })
		mockGetCollectionInvalidationTags.mockReturnValue(["collections-list", "collection-bague-soleil"])
	...
	it("should return auth error when not admin", async () => {
		mockRequireAdmin.mockResolvedValue({ error: { status: ActionStatus.UNAUTHORIZED, message: "Non autorise" } })
...
describe("removeFeaturedProduct", () => {
	beforeEach(() => {
		...
		mockValidateInput.mockReturnValue({ data: { collectionId: COLLECTION_ID, productId: PRODUCT_ID } })
		mockGetCollectionInvalidationTags.mockReturnValue(["collections-list", "collection-bague-soleil"])
		mockPrisma.productCollection.findUnique.mockResolvedValue(makeProductCollection({ isFeatured: true }))
