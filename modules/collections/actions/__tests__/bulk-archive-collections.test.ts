...
vi.mock("@/shared/lib/actions", () => ({
	...
	notFound: (entity: string) => ({ status: ActionStatus.NOT_FOUND, message: `${entity} introuvable` }),
}))
...
describe("bulkArchiveCollections", () => {
	...
	it("should return error for malformed JSON collectionIds", async () => {
		const formData = createMockFormData({ collectionIds: "not-valid-json", targetStatus: "ARCHIVED" })
	...
	it("should return not found when some collections do not exist", async () => {
		mockValidateInput.mockReturnValue({ data: { collectionIds: VALID_IDS, targetStatus: "ARCHIVED" } })
		mockPrisma.collection.findMany.mockResolvedValue([
			createMockCollection({ id: VALID_CUID }),
		])
	...
	it("should restore collections when targetStatus is PUBLIC", async () => {
		...
		mockValidateInput.mockReturnValue({ data: { collectionIds: [VALID_CUID], targetStatus: "PUBLIC" } })
		mockPrisma.collection.findMany.mockResolvedValue(collections)
	...
	it("should return collections in success data", async () => {
		const collections = [createMockCollection({ id: VALID_CUID, name: "Ma Collection", slug: "ma-collection" })]
		mockValidateInput.mockReturnValue({ data: { collectionIds: [VALID_CUID], targetStatus: "ARCHIVED" } })
		mockPrisma.collection.findMany.mockResolvedValue(collections)
