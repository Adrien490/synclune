...
import {
	quickSearchCollectionsAdmin,
	type AdminQuickSearchCollectionItem,
} from "../data/quick-search-collections-admin";

import type { AdminQuickSearchResult } from "@/shared/components/sticky-action-bar";

const querySchema = z.string().trim().min(1).max(100);

export async function quickSearchCollectionsAdminAction(
	query: string,
): Promise<AdminQuickSearchResult<AdminQuickSearchCollectionItem>> {
	...
	const result = await quickSearchCollectionsAdmin(parsed.data);
