...
describe("revokeSession", () => {
	...
	it("should return auth error when not authenticated", async () => {
		mockRequireAuth.mockResolvedValue({ error: { status: ActionStatus.UNAUTHORIZED, message: "Non authentifié" } })
	...
	it("should call getAuthSessionInvalidationTags with sessionId and userId", async () => {
		await revokeSession(undefined, validFormData)
		expect(mockGetAuthSessionInvalidationTags).toHaveBeenCalledWith(VALID_SESSION_ID, VALID_USER_ID)
	})
