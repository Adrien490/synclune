...
import {
	quickSearchProductTypesAdmin,
	type AdminQuickSearchProductTypeItem,
} from "../data/quick-search-product-types-admin";

import type { AdminQuickSearchResult } from "@/shared/components/sticky-action-bar";

const querySchema = z.string().trim().min(1).max(100);

export async function quickSearchProductTypesAdminAction(
	query: string,
): Promise<AdminQuickSearchResult<AdminQuickSearchProductTypeItem>> {
	...
	const result = await quickSearchProductTypesAdmin(parsed.data);
