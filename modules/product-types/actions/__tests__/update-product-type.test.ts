...
vi.mock("@/shared/lib/rate-limit-config", () => ({ ADMIN_PRODUCT_TYPE_LIMITS: { UPDATE: "pt-update" } }))
...
const validFormData = createMockFormData({ id: "pt-1", label: "Bague Updated", description: "Nouvelle description" })
...
describe("updateProductType", () => {
	beforeEach(() => {
		...
		mockValidateInput.mockReturnValue({ data: { id: "pt-1", label: "Bague Updated", description: "Nouvelle description" } })
	...
	it("should return error when new label already exists on another type", async () => {
		...
		const result = await updateProductType(undefined, validFormData)
		expect(mockError).toHaveBeenCalledWith("Ce label de type existe deja. Veuillez en choisir un autre.")
		expect(result.status).toBe(ActionStatus.ERROR)
	})

	it("should skip duplicate label check when label is unchanged", async () => {
		mockValidateInput.mockReturnValue({ data: { id: "pt-1", label: "Bague", description: "Description" } })
	...
	it("should keep existing slug when label is unchanged", async () => {
		mockValidateInput.mockReturnValue({ data: { id: "pt-1", label: "Bague", description: "Description" } })
		mockPrisma.productType.findUnique.mockResolvedValue(makeProductType({ label: "Bague", slug: "bague" }))
		await updateProductType(undefined, validFormData)
	...
	it("should update type with sanitized label and description", async () => {
		mockSanitizeText.mockReturnValueOnce("Bague Updated sanitized").mockReturnValueOnce("Nouvelle description sanitized")
		await updateProductType(undefined, validFormData)
		expect(mockPrisma.productType.update).toHaveBeenCalledWith({
			where: { id: "pt-1" },
	...
	it("should set description to null when not provided", async () => {
		mockValidateInput.mockReturnValue({ data: { id: "pt-1", label: "Bague Updated", description: undefined } })
		await updateProductType(undefined, validFormData)
		expect(mockPrisma.productType.update).toHaveBeenCalledWith({
			where: { id: "pt-1" },
			data: expect.objectContaining({
