...
vi.mock("@/shared/lib/rate-limit-config", () => ({ ADMIN_PRODUCT_TYPE_LIMITS: { DELETE: "pt-delete" } }))
...
describe("deleteProductType", () => {
	...
	it("should return error when product type has active products", async () => {
		mockPrisma.productType.findUnique.mockResolvedValue(makeProductType({ _count: { products: 3 } }))
		const result = await deleteProductType(undefined, validFormData)
		expect(result.status).toBe(ActionStatus.ERROR)
