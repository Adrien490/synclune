...
describe("TaxonomyFilterSheet — product-type", () => {
	beforeEach(() => {
	...
	it("renders the filter sheet wrapper", () => {
		render(<TaxonomyFilterSheet config={TAXONOMY_CONFIG["product-type"]} />);
	...
	it("forwards className to FilterSheetWrapper as triggerClassName", () => {
		render(<TaxonomyFilterSheet config={TAXONOMY_CONFIG["product-type"]} className="my-class" />);
	...
	it("does not bubble the submit event to the parent", () => {
		...
		render(
			<div onSubmit={parentSubmit}>
				<TaxonomyFilterSheet config={TAXONOMY_CONFIG["product-type"]} />
			</div>,
		);
		const form = document.querySelector("form")!;
		fireEvent.submit(form);
