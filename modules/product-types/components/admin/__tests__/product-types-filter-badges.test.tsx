...
vi.mock("@/shared/utils/format-status-filter", () => ({
	formatStatusFilter: (value: string, activeLabel: string, inactiveLabel: string) => ({
		label: "Statut actif",
...
import { ProductTypesFilterBadges } from "../product-types-filter-badges";
...
describe("ProductTypesFilterBadges", () => {
	it("renders the FilterBadges component", () => {
		render(<ProductTypesFilterBadges />);
	...
	describe("formatFilter logic", () => {
		...
		it("formats isActive filter with 'Actif' display value when value is 'true'", () => {
		...
		it("formats isActive filter with 'Inactif' display value when value is 'false'", () => {
			const formatFilter = getFormatFilter();
