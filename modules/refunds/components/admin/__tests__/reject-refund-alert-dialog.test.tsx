...
vi.mock("@/modules/refunds/hooks/use-reject-refund", () => ({
	useRejectRefund: ({ onSuccess }: { onSuccess?: () => void } = {}) => ({
		state: mockState,
		action: mockAction,
...
describe("RejectRefundAlertDialog", () => {
	beforeEach(() => {
		cleanup();
		vi.clearAllMocks();
		mockIsPending = false;
	...
	it("renders the dialog title", () => {
		...
		expect(screen.getByText("Refuser le remboursement")).toBeInTheDocument();
	});
	...
	it("shows rejection reason textarea with correct name attribute", () => {
		render(<RejectRefundAlertDialog />);

		const textarea = document.querySelector('textarea[name="reason"]') as HTMLTextAreaElement;
		expect(textarea).toBeTruthy();
		expect(textarea.id).toBe("reject-reason");
	});
	...
	it("does not show error message when state has SUCCESS status", () => {
		mockState = { status: "success", message: "Remboursement refusé" };

		render(<RejectRefundAlertDialog />);

		expect(screen.queryByText("Remboursement refusé")).not.toBeInTheDocument();
	});
	...
	it("exports the correct REJECT_REFUND_DIALOG_ID constant", () => {
		expect(REJECT_REFUND_DIALOG_ID).toBe("reject-refund");
	});
});
