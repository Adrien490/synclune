...
describe("buildRefundWhereClause", () => {
	...
	describe("filters", () => {
		it("should filter by single status", () => {
			const result = buildRefundWhereClause(params({ filters: { status: "PENDING" as never } }));
			const andArray = result.AND as Record<string, unknown>[];
		...
		it("should filter by single reason", () => {
			const result = buildRefundWhereClause(params({ filters: { reason: "DEFECTIVE" as never } }));
			const andArray = result.AND as Record<string, unknown>[];
		...
		it("should filter by orderId", () => {
			const result = buildRefundWhereClause(params({ filters: { orderId: "order-123" } }));
			const andArray = result.AND as Record<string, unknown>[];
		...
		it("should filter by createdAfter", () => {
			...
			const result = buildRefundWhereClause(params({ filters: { createdAfter: date } }));
			const andArray = result.AND as Record<string, unknown>[];
			expect(andArray).toContainEqual({
				createdAt: { gte: date },
			});
		...
		it("should filter by createdBefore", () => {
			...
			const result = buildRefundWhereClause(params({ filters: { createdBefore: date } }));
			const andArray = result.AND as Record<string, unknown>[];
			expect(andArray).toContainEqual({
				createdAt: { lte: date },
			});
