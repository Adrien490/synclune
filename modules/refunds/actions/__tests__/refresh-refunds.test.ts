...
vi.mock("@/shared/lib/rate-limit-config", () => ({ REFUND_LIMITS: { REFRESH: "refund-refresh" } }))
...
describe("refreshRefunds", () => {
	...
	it("should return auth error when not admin", async () => {
		...
		const result = await refreshRefunds(undefined, mockFormData)
	...
	it("should invalidate orders list and admin badges cache tags on success", async () => {
		mockSuccess.mockReturnValue({ status: ActionStatus.SUCCESS, message: "Remboursements rafraichis" })
	...
	it("should return success after cache invalidation", async () => {
		const result = await refreshRefunds(undefined, mockFormData)
		expect(mockSuccess).toHaveBeenCalledWith("Remboursements rafraichis")
