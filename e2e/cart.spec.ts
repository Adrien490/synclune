...
test.describe("Panier", { tag: ["@critical"] }, () => {
	...
	test("l'attribut aria-expanded du bouton panier reflète l'état du sheet", async ({ cartPage }) => {
		// Initialement fermé
	...
	test("le badge panier affiche 0 ou aucun badge quand le panier est vide", async ({ cartPage }) => {
		await expect(cartPage.openButton).toBeVisible()
