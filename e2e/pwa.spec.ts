...
test.describe("PWA - Offline et Service Worker", { tag: ["@slow"] }, () => {
	...
	test("les icones PWA sont accessibles", async ({ page }) => {
		...
		for (const iconPath of iconPaths) {
			...
			expect(response?.status(), `PWA icon ${iconPath} should be accessible`).toBe(200);
		}
	});

	test("les icones maskable sont declarees dans le manifest", async ({ page }) => {
		const response = await page.goto("/manifest.webmanifest");
		...
		const maskableSizes = maskableIcons.map((icon: { sizes: string }) => icon.sizes);
		expect(maskableSizes).toContain("192x192");
		expect(maskableSizes).toContain("512x512");
	});

	test("les screenshots du manifest sont accessibles", async ({ page }) => {
		const response = await page.goto("/manifest.webmanifest");
		const manifest = JSON.parse((await response?.text())!);
		...
		for (const screenshot of manifest.screenshots) {
			const screenshotResponse = await page.goto(screenshot.src);
			expect(
				screenshotResponse?.status(),
				`Screenshot ${screenshot.src} should be accessible`,
			).toBe(200);
		}
	});
