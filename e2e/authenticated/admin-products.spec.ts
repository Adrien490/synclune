...
test.describe("Admin - Gestion des produits", () => {
	...
	test("les onglets de statut sont visibles", async ({ page, adminPage }) => {
		...
		const statusFilters = page.getByRole("tab").or(page.getByRole("button", { name: /Tous|Actif|Brouillon|Archivé/i }))
	...
	test("le formulaire de creation produit contient les champs requis", async ({ page, adminPage }) => {
		await adminPage.gotoProducts()
