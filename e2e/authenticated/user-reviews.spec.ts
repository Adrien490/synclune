...
test.describe("Avis produits", { tag: ["@regression"] }, () => {
	test("les avis sont visibles sur la page detail produit", async ({
		page,
		productCatalogPage,
	}) => {
		...
		await expect(reviewsHeading.first().or(emptyReviewsState.first())).toBeVisible({
			timeout: 5000,
		});
	});

	test("le formulaire d'avis est accessible pour les produits commandes", async ({
		page,
		orderPage,
	}) => {
		...
		const reviewButton = page
			.getByRole("button", { name: /avis|évaluer|noter/i })
	...
	test("le formulaire d'avis contient les champs requis", async ({ page }) => {
		...
		test.skip(
			!ratingVisible,
			"Review form not available - user may not have purchased this product",
		);
